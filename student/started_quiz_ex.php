...
<body>
    ...
    <?php 
        if ((isset($_POST['submit_next'])) && (isset($_POST['user_ans'])))  {
            $query_ca = mysqli_query($link, "select * from result_experiment where id = '$_POST[result_id]' ");
            while($data_ca = mysqli_fetch_array($query_ca)) {
                $correct_ans = $data_ca['correct_ans'];
            }

            if (($_POST['user_ans']==$correct_ans) && ($_POST['user_ans']!='')) {                
                $upd = "UPDATE result_experiment SET given_ans='$_POST[user_ans]', question_mark='1', sts='1' WHERE id = '$_POST[result_id]'";
                mysqli_query ($link, $upd);
            } else {
                $upd = "UPDATE result_experiment SET given_ans='$_POST[user_ans]', question_mark='0', sts='2' WHERE id = '$_POST[result_id]'";
                mysqli_query ($link, $upd);
            }
        }
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body" >
                                        ...
                                        <?php 
                                            ...
                                            $query_chk = mysqli_query($link, "select * from result_experiment where unique_code = '$_GET[unique_code]' AND sts = '0' ");
                                            if (mysqli_num_rows($query_chk) < 1) {

                                                $your_mark = 0;
                                                $query_chk2 = mysqli_query($link, "select * from result_experiment where unique_code = '$_GET[unique_code]' ");
                                                while($data_chk2 = mysqli_fetch_array($query_chk2)) {
                                                    $your_mark = $your_mark+$data_chk2['question_mark'];
                                                }
                                                $upd = "UPDATE result_summery_experiment SET your_mark='$your_mark', sts='1' WHERE unique_code = '$_GET[unique_code]'";
                                                mysqli_query ($link, $upd);

                                                echo "<h6 style='color:Green;'>Your Exam Completed.</h6>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
                                                echo "<a href='result_ex.php'>View Results</a>";
                                                exit;
                                        ...
                                        <?php 
                                            $query2 = mysqli_query($link, "select * from result_experiment where experiment_id = '$_GET[experiment_id]' AND unique_code = '$_GET[unique_code]' AND sts = 0 order by sl asc limit 1");
                                            while($data2 = mysqli_fetch_array($query2)) {  

                                                $query3 = mysqli_query($link, "select * from question_experiment where id = '$data2[question_id]' ");
                                                while($data3 = mysqli_fetch_array($query3)) {

                                        ?>
                                            <font size="3"><b>
                                                <font color="#FF5733">Question <?= $data2['sl'] ?>. </font> <br>
                                                <?= $data3['des'] ?>
